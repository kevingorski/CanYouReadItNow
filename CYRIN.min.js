...
})[0].value+=h};a.prototype.addSuggestion=function(g,h){this.suggestions.push({message:g,priority:10-h})};a.prototype.addChild=function(g){this.children.push(g)};a.prototype.sort=function(){var g=function(i,h){return i.name>h.name};this.scores.sort(g);this.aggregateScores.sort(g);this.metrics.sort(g)};a.prototype.rollUp=function(){var g=this;if(this.children&&this.children.length){var i=this,h=this.textTree?this.textTree.getMetric(d.TEXT_LENGTH):0;i.totalCharacterCount=h;f.each(this.children,function(){i.totalCharacterCount+=this.rollUp()
...
f.extend(f.CYRIN,d);f.fn.CYRIN=function(m,i){var k=(new f.CYRIN(this,i));function g(E){return parseFloat(/(\d*\.*\d*)/.exec(E)[0])}function o(H){var G=k.findDPI(),F=g(H),E=G/72;return F/E}function n(E){return(E.css("font-weight")+"").match(/^bold.*|[7-9]00/)||E[0].nodeName.match(/strong/i)}function s(H){var F=H.width(),G;if((G=H.css("column-count")||H.css("-webkit-column-count")||H.css("-moz-column-count"))!=="auto"&&G>1){var E=g(H.css("column-gap")||H.css("-webkit-column-gap")||H.css("-moz-column-gap"));
