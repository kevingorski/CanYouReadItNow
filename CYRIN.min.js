...
})[0].value+=h};a.prototype.addSuggestion=function(g,h){this.suggestions.push({message:g,priority:10-h})};a.prototype.addChild=function(g){this.children.push(g)};a.prototype.sort=function(){var g=function(i,h){return i.name>h.name};this.scores.sort(g);this.aggregateScores.sort(g);this.metrics.sort(g)};a.prototype.applyToAll=function(h){h(this);if(!this.children.length){return}for(var g=this.children.length-1;g>=0;g--){this.children[g].applyToAll(h)}};a.prototype.rollUp=function(){var g=this;if(this.children&&this.children.length){var i=this,h=this.textTree?this.textTree.getMetric(d.TEXT_LENGTH):0;
i.totalCharacterCount=h;f.each(this.children,function(){i.totalCharacterCount+=this.rollUp()});if(i.totalCharacterCount>0){i.aggregateScores=f.map(i.scores,function(j){return{name:j.name,value:j.value*h/i.totalCharacterCount,description:j.description}})}f.each(this.children,function(){var k=this,j=k.totalCharacterCount/i.totalCharacterCount;f.each(i.aggregateScores,function(){var l=k.hasAggregateScore(this.name)?k:i;i.addToAggregate(this.name,(l.getAggregateScore(this.name)||0)*j)})});f.each(this.aggregateScores,function(){i.aggregateScore+=this.value;
this.value=b(this.value)});i.aggregateScore=b(i.aggregateScore)}else{this.totalCharacterCount=this.textTree.getMetric(d.TEXT_LENGTH);this.aggregateScores=this.scores;this.aggregateScore=this.score}this.sort();return this.totalCharacterCount};f.CYRIN=function(i,g){var j=this,h;j.el=i;f.CYRIN.defaultOptions.context=document.body;j.init=function(){j.options=f.extend({},f.CYRIN.defaultOptions,g)};j.findDPI=function(){if(h){return h}var k=f("<div />",{css:{position:"absolute",width:"1in"}}).appendTo(document.body);
h=k.width();k.remove();return h};j.init()};f.CYRIN.defaultOptions={};f.extend(f.CYRIN,d);f.fn.CYRIN=function(m,i){var k=(new f.CYRIN(this,i));function g(E){return parseFloat(/(\d*\.*\d*)/.exec(E)[0])}function o(H){var G=k.findDPI(),F=g(H),E=G/72;return F/E}function n(E){return(E.css("font-weight")+"").match(/^bold.*|[7-9]00/)||E[0].nodeName.match(/strong/i)}function s(H){var F=H.width(),G;if((G=H.css("column-count")||H.css("-webkit-column-count")||H.css("-moz-column-count"))!=="auto"&&G>1){var E=g(H.css("column-gap")||H.css("-webkit-column-gap")||H.css("-moz-column-gap"));
...
})}return G}if(!(k.el&&k.el.length)){return"Provide a target to analyze."}if(!jQuery.contains(k.el,k.options.context)){return"Target doesn't appear to be in the DOM."}var v=0,B=0,w,C,r=new a();k.el.each(function(){var G=f(this),J=new e(),I,F=0;w=u(G);if(!(C=A(G.css("background-color")))){var H=G;while(H&&H[0]!==document&&!(C=A(H.css("background-color")))){H=H.parent()}C=A(C)||"#ffffff"}I=G.parent();while(I[0]!=document&&I.css("display")==="block"){var E=I.css("margin-left")||0;F+=g(I.css("padding-left")||0);
